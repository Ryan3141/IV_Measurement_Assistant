...
class IV_Measurement_Assistant_App( QtWidgets.QWidget, Ui_MainWindow, Saveable_Session, Threaded_Subsystems ):
	...
	def Init_Subsystems( self ):
		...
		self.graph.set_labels( title="I-V", x_label="Voltage (V)", y_label="Current (A)" )
...
def Measurement_Sweep( quit_early,
	...
	for (set_temp, actual_temperature), (device, pads_info), _ in ((x,y,z) for x in run_temperatures for y in run_devices for z in turn_heater_back_on ):
		meta_data.update( dict( temperature_in_k=actual_temperature, device_location=device.location, device_side_length_in_um=device.side ) )
